...
def call_main():
    ...
    description = """CLIPper Input Norm. Michael Lovci, Gabriel Pratt, Eric Van Nostrand 2017.
                     ...
                     (but not identital due to numerical precision issues between python and the initial perl implementation
    ...
    parser.add_option("--ipbed",  dest="peaks", help="A peak file to input normalize", type="string")
    parser.add_option("--ipbam",  dest="ip_bam", help="a bam file of the IP reads to input normalze against")
    parser.add_option("--ipbai",  dest="ip_bai", help="a bam index file of the IP reads to input normalze against")
    parser.add_option("--inbam",  dest="input_bam", help="a bam file of the input reads to input normalze against")
    parser.add_option("--inbai",  dest="input_bai", help="a bam index file of the input reads to input normalze against")
    parser.add_option("--output", dest="out", default="norm_clusters.bed", help="a bed file output, default:%default")
    parser.add_option("--outputfull", dest="out_full", default="norm_clusters.full", help="A full file output (used for specific analysies), default:%default")

    (options, args) = parser.parse_args()
    run(pybedtools.BedTool(options.peaks),
        pysam.Samfile(options.ip_bam),
        pysam.Samfile(options.input_bam),
        options.out,
        options.out_full
        , True
        )
