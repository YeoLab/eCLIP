...
def _barcodedict_longest_barcode_length(barcodedict):
    """
    :param barcodedict: a dict with keys/values as barcode seq. / barcode id
    :return: int, length of longest barcode in barcodedict
    """
    return max([len(bcseq) for bcseq in barcodedict if bcseq])
...
def assignbarcode(readsequence, barcodedict, allowed_distance=0):
    ...
    Args:
        readsequence: str, read to check if barcode exists in
    ...
    Returns:
        if found: returns barcode seq best matching front of readsequence,
        if no barcode could be matched: returns the empty string ('')
    Throws error: if readsequence shorter then longest barcode in barcodedict
    """
    ###########################################################################
    # TODO ther approach was used in singke end version of demux
    # TODO " for each barcode length see if known barcodes appear "
    ###########################################################################
    ...
    #         break
    ###########################################################################
    # Gets min hamming distance between readsequence and barcodes in dict
    # if tie on min distance, assigned barcode will be first in ordered dict
    distances_to_bcseqs = [(hamming_loose(readsequence, bcseq),
                            bcseq)
...
def _pairedreads_reformat(fastqread1, fastqread2,
                          bcfoundlength, randomerlength):
    """
    reformat paired fastqread's to include randomesequence in title
    and cut barcode seq from front of read1
    and cut randomer seq from front of read2
    Args:
        fastqread1: fastq dict (with title, sequence and quality properties)
        fastqread2: fastq dict for other end pairing with fastqread1
        bcfoundlength: length of matched barcode to trim at front of fastqread1
    ...
    """
    bcfoundseq = fastqread1["sequence"][0:bcfoundlength]
    ...
    fastqread1 = _fastqread_trim(fastqread1, bcfoundlength)
    ...
    # no barcode sequence assigned:
    # if bcfoundlength is 0, bcseqfound will be "" and that is fine

    return fastqread1, fastqread2, bcfoundseq, randomerseq
...
def too_short(datastr, fastq1, fastq2, randomer_length):
   fastq1_tooshort = (
       len(fastq1["sequence"])
       <=  _barcodedict_longest_barcode_length(datastr.barcodedict))
   fastq2_tooshort = (
       (len(fastq2["sequence"])
        <= randomer_length))
   return fastq1_tooshort or fastq2_tooshort
...
def demux(datastr, fastq1, fastq2, randomer_length, allowed_distance):
    ...
    with _fqgz_open(fastq1) as fastqfile1, _fqgz_open(fastq2) as fastqfile2:
        while True:
            ...
            if not too_short(datastr, fastqread1, fastqread2, randomer_length):
                # legacy "too-short" id, what was bcassigned/bcfound/umi =?
                bcassigned, bcfound, umi = None, None, None

            else:
                bcassigned = assignbarcode(fastqread1["sequence"],
                                           datastr.barcodedict,
                                           allowed_distance)
                # bcassigned should be a string, possibly of length 0 ('')
                # bcassigned can be shorter then the longest barcode
                bcfound_length = len(bcassigned)
                fastqread1, fastqread2, bcfound, umi = _pairedreads_reformat(
                    fastqread1, fastqread2, bcfound_length, randomer_length)

                # unassigned to any barcode
                # legacy "unassigned" id, what was bcassigned/bcfound/umi =?
                if bcfound == '':
            ...
            # write reads with assigned barcode to that barcode demuxed files
            # write unassigned reads to demuxed "NIL"/"unassigned" files
            # write "tooshort" reads to demuxed "SHO" files
